...
export default function WorkerCard({ workerId, onDelete }) {
  const {
    workers,
    fetchWorkerById,   // non-mutating fetch
    updateWorker,       // context update
    loading,            // global loading indicator (optional)
    error,              // global error indicator (optional)
  } = useWorkers();

  // try to pick from global list first (cheap cache)
  const cached = useMemo(
    () => workers.find((w) => w.id === workerId),
    [workers, workerId]
  );

  const [local, setLocal] = useState(cached || null);
  const [busy, setBusy] = useState(false); // local loading for this card
  const [err, setErr] = useState(null);
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState({ name: "", job: "", phone: "", address: "" });

  // if not in cache, fetch on mount/workerId change (without mutating global selection)
  useEffect(() => {
    let cancelled = false;
    ...
    const ensureWorker = async () => {
      ...
      try {
        setBusy(true);
        const w = await fetchWorkerById(workerId);
        if (!cancelled) setLocal(w);
      } catch (e) {
        if (!cancelled) setErr("שגיאה בטעינת עובד");
        console.error(e);
      } finally {
  ...
  }, [workerId, cached, fetchWorkerById]);

  if (busy || (!local && (loading || !err))) return <div>טוען…</div>;
  if (err || error) return <div style={{ color: "red" }}>{err || "שגיאה בטעינת עובד"}</div>;
  if (!local) return null;
  ...
  const save = async () => {
    try {
      setBusy(true);
      const updated = await updateWorker(local.id, draft); // updates global state as well
      setLocal(updated); // keep local in sync
      setEditing(false);
      // no need for window.dispatchEvent(...) – Context updates all consumers
    } catch (e) {
      console.error(e);
      setErr("שמירה נכשלה");
    } finally {
  ...
  const actions = editing ? (
    <>
      <CustomPrimaryButton sx={{ width: "auto", px: 2 }} onClick={save} disabled={busy}>
        שמור
      </CustomPrimaryButton>
      <CustomPrimaryButton sx={{ width: "auto", px: 2, backgroundColor: "#64748b" }} onClick={cancelEdit}>
        ביטול
      </CustomPrimaryButton>
    </>
  ) : (
    <>
      <CustomPrimaryButton sx={{ width: "auto", px: 2 }} onClick={startEdit}>
        עריכה
      </CustomPrimaryButton>
      {onDelete && (
        <CustomPrimaryButton
